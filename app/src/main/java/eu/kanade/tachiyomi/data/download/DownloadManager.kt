...
class DownloadManager(
    ...
    private val nsfwContentFilter: NsfwContentFilter = Injekt.get(),
) {
    ...
    fun downloadChapters(manga: Manga, chapters: List<Chapter>, autoStart: Boolean = true) {
        if (!nsfwContentFilter.isMangaAllowed(manga)) return
    ...
    fun addDownloadsToStartOfQueue(downloads: List<Download>) {
        val allowedDownloads = downloads.filter { nsfwContentFilter.isMangaAllowed(it.manga) }
        if (allowedDownloads.isEmpty()) return
        queueState.value.toMutableList().apply {
            addAll(0, allowedDownloads)
            reorderQueue(this)
        }
